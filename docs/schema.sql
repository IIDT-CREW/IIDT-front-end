...
-- 1. 회원
-- ============================================
CREATE TABLE public.iidt_member (
...
CREATE TABLE public.iidt_will (
  ...
  guest_nickname  VARCHAR(30),                                       -- 비회원 닉네임
  guest_password  TEXT,                                              -- 비회원 비밀번호 hash
  reg_date        TIMESTAMPTZ     NOT NULL DEFAULT now(),
  edit_date       TIMESTAMPTZ     NOT NULL DEFAULT now()
);
...
CREATE INDEX idx_iidt_will_mem_idx ON public.iidt_will(mem_idx);
CREATE INDEX idx_iidt_will_list ON public.iidt_will(is_delete, reg_date DESC);
...
-- ============================================
CREATE TABLE public.iidt_will_answer (
...
CREATE INDEX idx_iidt_answer_will_id ON public.iidt_will_answer(will_id);


-- ============================================
-- 4. 토큰 블랙리스트 (로그아웃)
-- ============================================
CREATE TABLE public.iidt_token_blacklist (
...
CREATE INDEX idx_iidt_blacklist_expired_at ON public.iidt_token_blacklist(expired_at);
...
-- 필요 시 아래처럼 정책 추가:
--
-- CREATE POLICY "본인 유언장만 수정" ON public.iidt_will
--   FOR UPDATE USING (mem_idx = current_setting('app.current_user_idx')::int);
--
-- CREATE POLICY "공개 유언장 조회" ON public.iidt_will
